...
/* ---- Inicio (Home showcase/gallery media) -------------------------------- */
export async function getHomeMediaAdmin() {
  const rows = await prisma.homeMedia.findMany({ orderBy: [{ kind: "asc" }, { position: "asc" }] });
  return rows.map((r) => ({ id: r.id, kind: r.kind, position: r.position, imageUrl: r.imageUrl || "", label: r.label || "" }));
}

export async function getGalleryFeaturesAdmin() {
